...
def load_vectorstore(persist_directory: Path):
    ...
    return Chroma(persist_directory=str(persist_directory), collection_name="grants_tenders")
...
def main() -> None:
    ...
    history = load_chat_history(session_id="grantbot_session")
    ...
    response = agent.invoke({"input": user_query, "chat_history": []})
    print(response)
